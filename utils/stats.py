...
def calculate_p_values(
    ...
    Args:
        df_vars (pd.DataFrame):
            Pandas DataFrame with predictor columns.
        series_target (pd.Series):
            Pandas Series with the target variable.
        scale_function (str):
            Scaling function. Defaults to "z-score".
        imputer_strategy (str):
            Imputation strategy. 
             Options are 'mean', 'median', 'most_frequent', 'constant'.
             Defaults to 'mean'.
        model_type (str):
            Model type. Options are "OLS", "RobustLS", "GLM", "Quantile"
            Defaults to "OLS".
        transformation (str):
            Data transformation to apply. Options are "polynomial", "log",
            "sqrt", "reciprocal", "boxcox_y", "None". Defaults to "None".
    ...
    # Data transformations. Could be useful if data relationships are non-linear
    ...
    # Scaling (StandardScaler only). Finds non-binary columns, then scales
    ...
    # Imputation - uses the specified imputer strategy
    ...
    # Model Fitting and Metrics based on selected model method
    if model_type == "OLS":
    ...
    if p > 0.05: # Use > here to check for normality
        ...
        print("WARNING: Heteroscedasticity detected via Breusch-Pagan.")
    ...
    # 3. Linearity (Scatter plots of each predictor vs. the target)
    for col in df_vars.columns:
        plt.figure()
...
# ) -> pd.DataFrame:
#     """Evaluates and compares multiple regression models with different 
#     model and transformation types to see which combination has the 
#     best results. Used for investicative purposes only.

#     Args:
#         df_vars (pd.DataFrame):
#             DataFrame of predictor variables.
#         y (pd.Series):
#             Target variable. Long COVID rates
#         transformations (Optional[List[str]], optional):
#             List of transformations to apply. Defaults to ["None"].
            
#         model_types:
#             List of model types to scan. Defaults to OLS 
#             "OLS", "GAM", "RobustLS", "GLM", "Quantile"
            
#     Returns:
#         df_results (pd.DataFrame):
#             DataFrame containing correlation model evaluation results.
#     """
